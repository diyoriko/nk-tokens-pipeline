...
const byValue = (obj) =>
  Object.keys(obj).sort((a, b) => parseFloat(obj[a]) - parseFloat(obj[b]));

function rows(group, names) {
  const wrap = document.createElement('div');
  wrap.style.cssText = `padding:24px;font-family:${FONT};font-size:13px`;
  for (const name of names) {
    const v = cssVar(['size', group, name]);
    const px = parseFloat(tokens.size[group][name]);
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;align-items:center;gap:16px;margin-bottom:8px';
    const isFull = name === 'full';
    row.innerHTML = `
      <code style="width:260px;color:${MUTED}">var(${v})</code>
      <div style="height:16px;width:${Math.max(0, px)}px;max-width:520px;background:var(--nk-color-background-brand-violet-primary);${
        isFull ? 'width:120px;border-radius:var(--nk-size-radius-full)' : ''
      }"></div>
      <span style="color:${MUTED};font-variant-numeric:tabular-nums">${tokens.size[group][name]}</span>`;
    wrap.appendChild(row);
  }
  return wrap;
}

export const Space = () => rows('space', byValue(tokens.size.space));
export const Radius = () => {
  const wrap = document.createElement('div');
  wrap.style.cssText = `display:flex;gap:24px;flex-wrap:wrap;padding:24px;font-family:${FONT}`;
  for (const name of byValue(tokens.size.radius)) {
    const v = cssVar(['size', 'radius', name]);
    const cell = document.createElement('div');
    cell.style.cssText = 'text-align:center;font-size:12px';
    cell.innerHTML = `
      <div style="width:96px;height:96px;background:var(--nk-color-background-brand-violet-secondary);border:1px solid ${BORDER};border-radius:var(${v})"></div>
