...
| Browsing the system visually | the Storybook (`npm run storybook`; deployed from `develop` via GitHub Pages) |
| Using the tokens as a designer | the **designer guide** — generated by `../scripts/build-guide.mjs`, published at `/guide/` alongside the Storybook |
| Code → Figma procedures (modes, scopes, drift) | [`../figma/RUNBOOK.md`](../figma/RUNBOOK.md) |
| Release history | [`../CHANGELOG.md`](../CHANGELOG.md) |

## Still-current docs in this folder

One doc here is a living reference, not archive:

- [`CAPSULES.md`](CAPSULES.md) — the per-team capsule model: shape, gates,
  add/consume checklists. **Maintainer-facing** (registry, build, npm subpaths);
  the designer-facing half lives in the guide.

`BUILD-ON-PRIMITIVES.md` was removed 2026-07-23: it and `CAPSULES.md` gave two
different answers to the same question ("a team wants its own semantics"). Both
answers are real, so they are now one decision tree in the **designer guide**
(`scripts/build-guide.mjs`, published at `/guide/` next to the Storybook) —
own Figma collection vs capsule, with the trade-offs stated.
