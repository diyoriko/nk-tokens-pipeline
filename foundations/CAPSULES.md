...
- **Parent Area** is the **base brand overlay** — it holds the default (violet)
  ...
  (`@novakid/design-system` `.` = core + Parent Area) *and* under every other team's capsule.
...
The default build (`@novakid/design-system` `.` entry → `build/css|dart|ts`) is
...
```js
import tokens from '@novakid/design-system/capsules/team-b';        // primitives + Team B semantics
// or CSS:
// @import '@novakid/design-system/capsules/team-b/css/variables.css';
```
