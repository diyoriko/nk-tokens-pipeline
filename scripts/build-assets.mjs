...
// CJS SIBLINGS. `./icons` and `./icons/react` were ESM-only, which is fine for a bundler
// and fatal for a test runner: novakid-parent-mf's Jest uses tsconfig.jest.json with
// `"module": "commonjs"` + `"moduleResolution": "node"` and no transformIgnorePatterns, so
// the first component test that imports an icon dies with
// `SyntaxError: Cannot use import statement outside a module`. That is the first consumer,
// on the first test — the audit's finding 4.8.
//
// The CJS barrel is deliberately SELF-CONTAINED rather than 160 more require() files:
// CommonJS consumers here are test runners and legacy builds where bundle size does not
// matter, and tree-shaking does not apply to `require` anyway. The ESM path — the one a
// bundler actually walks — keeps its per-module shape.
const CJS_FACTORY = `const React = require('react');

const make = (name, body) => {
  const Icon = React.forwardRef(function NkIcon(props, ref) {
    const { title, children, ...rest } = props || {};
    const labelled = title != null || rest['aria-label'] != null || rest['aria-labelledby'] != null;
    const a11y = labelled ? { role: 'img' } : { 'aria-hidden': true, focusable: false };
    return React.createElement(
      ...
      title != null ? React.createElement('title', null, title) : null,
      React.createElement('g', { dangerouslySetInnerHTML: { __html: body } })
    );
  });
  Icon.displayName = name;
  return Icon;
...
fs.writeFileSync(
  B('icons/react.cjs'),
  '// Generated by build-assets.mjs. CommonJS sibling of react.js, self-contained by design\n' +
    '// (see the comment in scripts/build-assets.mjs). Consumers that `require` this get every\n' +
    '// icon; consumers that `import` walk the per-module ESM path and pay for what they use.\n' +
    CJS_FACTORY +
    manifest
      .map(({ name }) => `exports.${pascal(name)} = make(${JSON.stringify(name)}, ${JSON.stringify(rawMap[name])});`)
      .join('\n') +
    '\n'
);
fs.writeFileSync(
  ...
  `import * as React from 'react';\n` +
    `export interface NkIconProps extends React.SVGProps<SVGSVGElement> { title?: string; }\n` +
    `export type NkIcon = React.ForwardRefExoticComponent<NkIconProps & React.RefAttributes<SVGSVGElement>>;\n` +
...
fs.writeFileSync(
  B('icons/index.cjs'),
  `// Generated by build-assets.mjs. Raw inner-SVG per icon (24x24, currentColor), CommonJS.\n` +
    `exports.icons = ${JSON.stringify(rawMap)};\nexports.iconNames = ${JSON.stringify(manifest.map((m) => m.name))};\n`
);
fs.writeFileSync(
  B('icons/index.d.cts'),
  `export type NkIconName = ${manifest.map((m) => `'${m.name}'`).join(' | ') || 'never'};\n` +
